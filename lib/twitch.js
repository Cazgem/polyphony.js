...
// Twitch.prototype.confirm = async function (modules, config = ['channel', 'channels']) {
...
Twitch.prototype.subscription = function (channel, user_id, callback) {
    const that = this;
    this.fetchAccessToken(channel, function (err, token) {
        const opts = {
            method: `GET`,
            url: `https://api.twitch.tv/helix/subscriptions?broadcaster_id=${channel}&user_id=${user_id}`,
            headers: {
                'Authorization': `Bearer ${token}`,
                'Client-ID': `${that.client_id}`
            }
...
Twitch.prototype.twitter = function () {
    console.log(`Twitch.twitter test`)
    Twitter.cross();
}